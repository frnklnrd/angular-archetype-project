...
{
  private readonly key: AuthProviderType | string = this.getProviderKey();
  ...
  public setAuthManagerService(authManager: AuthManagerService): void {
    this.logger.console.debug(this.__classname, 'setAuthManagerService', authManager);
  ...
  public abstract getProviderKey(): AuthProviderType | string;
  ...
  protected emitRefreshTokenCallback(data?: any): void {
    this.logger.console.debug(this.__classname, 'emitRefreshTokenCallback', data);
    this.logger.console.debug(
      this.__classname,
  ...
  public logout(data?: any): Promise<boolean> {
    ...
    this.doLogout(data).then(
      (result: boolean) => {
        this.logger.console.debug(this.__classname, 'logout -> successfully', result);
      },
      (error: any) => {
  ...
  public refreshToken(data?: any): Promise<boolean> {
    ...
    this.doRefreshToken(data).then(
      ...
      (error: any) => {
        this.logger.console.error(this.__classname, 'refreshToken -> error', error);
      }
    );
    return Promise.resolve(true);
  }
  ...
  public getTokenDecodedData(payload?: any): any {
    ...
    this.logger.console.debug(this.__classname, 'getTokenDecodedData -> token', token);
    if (!token) {
      return null;
    }
    try {
  ...
  public getAuthFullDataFromPayload(payload?: any): AuthFullDataModel {
    return {
      payload: payload,
  ...
  protected getUserDataFromPayload(payload?: any): AuthUserDataModel {
    return {
