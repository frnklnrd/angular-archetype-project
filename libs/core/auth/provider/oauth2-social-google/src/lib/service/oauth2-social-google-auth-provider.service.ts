...
export class Oauth2SocialGoogleAuthProviderService extends AbstractAuthProviderService<GoogleAuthInitializeConfigModel> {
  ...
  public getProviderKey(): AuthProviderType | string {
    return AuthProviderType.OAUTH2_SOCIAL_GOOGLE;
  }
  ...
  protected doConfigure(callback?: (data?: any) => void): Promise<boolean> {
    ...
    this.setAuthConfig(
      this.store.selectSnapshot<GoogleAuthInitializeConfigModel>(
        (state) => state.config.auth.providers.social.google.config
      )
    );
    ...
    return of(true)
      .pipe(
        ...
        map((a) => {
          try {
            this.logger.console.debug(this.__classname, 'GoogleAuth.initialize()');
            GoogleAuth.initialize({
              clientId: this.authConfig.clientId,
              scopes: this.authConfig.scopes,
              grantOfflineAccess: this.authConfig.grantOfflineAccess,
            });
            this.logger.console.debug(this.__classname, 'GoogleAuth.initialize() - ok');
          } catch (error) {
