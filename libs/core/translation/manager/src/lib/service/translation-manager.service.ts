...
@Injectable()
export class TranslationManagerService extends AbstractService {
  ...
  protected onChangedDefaultLang(lang: string): void {
    if (this.defaultLang !== lang) {
      ...
      this.store.dispatch([
        new TranslationChangeDefaultLangAction(this.defaultLang),
      ]);
    }
  }
  ...
  protected onChangedCurrentLang(lang: string): void {
    if (this.currentLang !== lang) {
      ...
      this.store.dispatch([
        new TranslationChangeCurrentLangAction(this.currentLang),
      ]);
    }
  }
}
